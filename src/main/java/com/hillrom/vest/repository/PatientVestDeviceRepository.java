...
public interface PatientVestDeviceRepository extends
	...
	@Query("from PatientVestDeviceHistory pvd where pvd.patientVestDevicePK.serialNumber = ?1  and pvd.patientVestDevicePK.active = true")
	...
	@Query("from PatientVestDeviceHistory pvd where pvd.patientVestDevicePK.patient.id = ?1 and pvd.patientVestDevicePK.active = ?2")
	Optional<PatientVestDeviceHistory> findOneByPatientIdAndActiveStatus(
			String patientId, Boolean active);

	@Query("from PatientVestDeviceHistory pvd where pvd.bluetoothId = ?1 and pvd.patientVestDevicePK.active = true")
