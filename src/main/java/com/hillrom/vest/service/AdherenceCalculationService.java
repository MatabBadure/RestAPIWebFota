...
public class AdherenceCalculationService {
	...
	public String adherenceSettingForClinic(String clinicId){
		try{
			...
			if(userList.size()>0){
				...
				System.out.println("adherenceSettingForClinic method executed in :"+totalTime+" milliseconds");
	...
	public List<TherapySession> getTherapyForDay(SortedMap<LocalDate,List<TherapySession>> sortedTherapySession,LocalDate curDate){
		...
		for(LocalDate date : sortedTherapySession.keySet()){
			if(date == curDate){
				for(List<TherapySession> therapySession : sortedTherapySession.values()){
					session.addAll(therapySession);					
				}
			}
		}
		return session;
	...
	public PatientCompliance returnPrevDayCompli(List<PatientCompliance> complianceList, LocalDate currDate){
		...
		//List<PatientCompliance> result = 
				//complianceList.stream().filter(PatientCompliance->PatientCompliance.date.equals(currDate)).collect(Collectors.toList());
		//List<PatientCompliance> revisions = 
				// complianceList.stream().filter(rev -> Objects.nonNull(rev.getDate().equals(currDate.minusDays(1)))).collect(Collectors.toList());
				//return prevDayCompli;
		
		
		/*PatientCompliance prevDayCompli = null;
		boolean flag = false;
		for(PatientCompliance compliance : complianceList){
			if(!flag && compliance.getDate() == currDate){				
				flag = true;
			}
			if(flag){
				prevDayCompli = compliance;	
			}			
		}
		return prevDayCompli;*/
				...
				if( ( adherenceStartDate.isBefore(currentCompliance.getDate()) || adherenceStartDate.equals(currentCompliance.getDate())) &&
					...
					if(currentCompliance.getMissedTherapyCount() >= adherenceSettingDay && !currentCompliance.getDate().equals(todayDate)){
						...
						calculateUserHMRComplianceForMST(currentCompliance, userProtocolConstant, currentCompliance.getDate(), userId, 
								patient, patientUser, adherenceSettingDay, initialPrevScoreFor1Day,sortedTherapy, prevCompliance, existingNotificationofTheDay);
					}
				}
			}
		}catch(Exception ex){
	...
	private void calculateUserHMRComplianceForMST(
		...
		notificationService.createOrUpdateNotification(patientUser, patient, userId,
				complianceDate, (initialPrevScoreFor1Day == 0 ? notification_type : ADHERENCE_SCORE_RESET), false, existingNotificationofTheDay);
