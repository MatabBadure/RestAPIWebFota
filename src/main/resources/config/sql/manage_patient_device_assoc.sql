...
DECLARE monarch_device_patient_type VARCHAR(50);
...
IF operation_type_indicator = 'CREATE' THEN
	...
	START TRANSACTION;
		...
		INSERT INTO `PATIENT_DEVICES_ASSOC`
			...
			VALUES
			(pat_patient_id,pat_device_type,1,pat_device_serial_number,pat_hillrom_id,temp_pat_patient_type,today_date,null,pat_old_id,pat_training_date,
...
ELSEIF operation_type_indicator ='UPDATE' THEN
		...
		SELECT `patient_id`, `serial_number` ,`patient_type` ,`device_type` INTO temp_patient_info_id, temp_serial_number 
	    FROM `PATIENT_DEVICES_ASSOC`
		WHERE `serial_number` = pat_device_serial_number  AND `patient_id` = pat_patient_id;
		...
		IF temp_serial_number =  pat_device_serial_number AND  monarch_device_patient_type ='SD' AND monarch_device_device_type = 'MONARCH' THEN
		
			SELECT `hillromid`,`patient_id` INTO vest_device_hillrom_id ,`vest_device_patient_id` FROM `PATIENT_DEVICES_ASSOC` WHERE `hillromid` = pat_hillrom_id AND `device_type` = 'VEST' AND `patient_type` = 'SD';
			...
			IF  temp_patient_info_id IS NOT NULL THEN
				START TRANSACTION;
					...
					AND  PVDA.`hillrom_id` = null AND PVDA. `patient_type` = monarch_device_patient_type AND PVDA.`device_type` = 'MONARCH' ;
