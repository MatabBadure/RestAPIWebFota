...
BEGIN
	...
	DECLARE temp_global_hmr_compliant INT DEFAULT 0;
	DECLARE temp_global_settings_deviated INT DEFAULT 0;
	DECLARE temp_global_missed_therapy_count INT DEFAULT 0;
	...
	INSERT INTO each_record_processed_validation (`id`, `processed`, `pc_date`) SELECT id, 0, date FROM PATIENT_COMPLIANCE WHERE user_id = @temp_user_id;
		...
		WHILE EXISTS(SELECT `id` FROM each_record_processed_validation WHERE processed = 0 LIMIT 1) DO
			...
			IF temp_is_hmr_compliant = 1 THEN 
				SET temp_global_hmr_compliant = temp_global_hmr_compliant + 1;
			END IF;

			IF temp_settings_deviated_days_count > 3 THEN 
				SET temp_settings_deviated_days_count = temp_global_settings_deviated +(temp_settings_deviated_days_count - 3);
			ELSE
				SET temp_settings_deviated_days_count = temp_global_settings_deviated + temp_settings_deviated_days_count;
			END IF;

			IF temp_missed_therapy_count >= 1 THEN 
				SET temp_global_missed_therapy_count = temp_global_missed_therapy_count + 1;
			END IF;


			UPDATE `PATIENT_COMPLIANCE` SET `global_hmr_compliant` = temp_global_hmr_compliant ,
			`global_settings_deviated` = temp_global_settings_deviated,
			`global_missed_therapy_count` = temp_global_missed_therapy_count WHERE `id` = @temp_pc_id;
		...
		SET temp_global_hmr_compliant = 0 ;
		SET temp_global_settings_deviated = 0;
		SET temp_global_missed_therapy_count = 0;
	END WHILE;
